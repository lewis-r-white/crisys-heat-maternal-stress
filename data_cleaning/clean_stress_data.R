...
pss_clean <- bind_rows(stress_data, pssfup_no_pss, pssfup_with_pss) %>%
  ...
  mutate(survey_time = case_when(is.na(quessetdt) & !is.na(time) ~ time,
                                 ...
                                 TRUE ~ NA)) %>%
  dplyr::select(-c(quessetd, quessetdt, interviewdate, time)) %>%
  mutate(
...
pss_recode <- pss_clean %>%
  ...
  mutate(
    PSS4 = ifelse(rowSums(is.na(dplyr::select(., recode_psa, recode_psb, recode_psc, recode_psd))) > 1, NA, rowSums(dplyr::select(., recode_psa, recode_psb, recode_psc, recode_psd), na.rm = TRUE))
  ) %>%
  
  mutate(
    PSS4_q3_binary = ifelse(PSS4 >= quantile(PSS4, 0.75, na.rm = TRUE), "low", "high"),
    PSS4_q3_binary = factor(PSS4_q3_binary, levels = c("low", "high")) 
  ) %>%
  
  left_join(delivery_date) %>% 
  
  # add difference in time between delivery date and date of survey
  mutate(diffdays = difftime(survey_date, datdeliv, units = "days")) %>%
  ...
  mutate(
    ...
    gestage_at_survey = gestage_days + as.numeric(difftime(survey_date, datdeliv, units = "days")),
...
crisis_recode <- crisis_recode %>%
  ...
  mutate(
    fin_events = rowSums(dplyr::select(., a2, a3, a5, a6, a7, a8, a9, a10, a11, a12, a14) == 1, na.rm = TRUE),
    crifinneg = rowSums(dplyr::select(., b2, b3, b5, b6, b7, b8, b9, b10, b11, b12, b14) == 1, na.rm = TRUE),
    fin_nds = ifelse(crifinneg > 0, 1, 0),
    
    leg_events = rowSums(dplyr::select(., a15, a16, a17, a18, a19) == 1, na.rm = TRUE),
    crilegalneg = rowSums(dplyr::select(., b15, b16, b17, b18, b19) == 1, na.rm = TRUE),
    leg_nds = ifelse(crilegalneg > 0, 1, 0),
    
    car_events = rowSums(dplyr::select(., a27, a73, a75) == 1, na.rm = TRUE),
    cricareerneg = rowSums(dplyr::select(., b27, b73, b75) == 1, na.rm = TRUE),
    car_nds = ifelse(cricareerneg > 0, 1, 0),
    
    rel_events = rowSums(dplyr::select(., a20, a21, a22, a24, a29, a30, a31, a32, a33, a34, a35, a36, a37) == 1, na.rm = TRUE),
    crirelneg = rowSums(dplyr::select(., b20, b21, b22, b24, b29, b30, b31, b32, b33, b34, b35, b36, b37) == 1, na.rm = TRUE),
    rel_nds = ifelse(crirelneg > 0, 1, 0),
    
    homesf_events = rowSums(dplyr::select(., a39, a40, a41) == 1, na.rm = TRUE),
    crihomesafeneg = rowSums(dplyr::select(., b39, b40, b41) == 1, na.rm = TRUE),
    homesf_nds = ifelse(crihomesafeneg > 0, 1, 0),
    
    neighsf_events = rowSums(dplyr::select(., a42, a43, a44, a45, a46, a47, a48, a38) == 1, na.rm = TRUE),
    crineighsafeneg = rowSums(dplyr::select(., b42, b43, b44, b45, b46, b47, b48, b38) == 1, na.rm = TRUE),
    neighsf_nds = ifelse(crineighsafeneg > 0, 1, 0),
    
    medself_events = rowSums(dplyr::select(., a52, a54, a55) == 1, na.rm = TRUE),
    crimedselfneg = rowSums(dplyr::select(., b52, b54, b55) == 1, na.rm = TRUE),
    medself_nds = ifelse(crimedselfneg > 0, 1, 0),
    
    medoth_events = rowSums(dplyr::select(., a53, a56, a57, a58) == 1, na.rm = TRUE),
    crimedothneg = rowSums(dplyr::select(., b53, b56, b57, b58) == 1, na.rm = TRUE),
    medoth_nds = ifelse(crimedothneg > 0, 1, 0),
    
    home_events = rowSums(dplyr::select(., a59, a60, a61, a62, a63, a64, a13) == 1, na.rm = TRUE),
    crihomeneg = rowSums(dplyr::select(., b59, b60, b61, b62, b63, b64, b13) == 1, na.rm = TRUE),
    home_nds = ifelse(crihomeneg > 0, 1, 0),
    
    prej_events = rowSums(dplyr::select(., a66, a67, a68, a69, a70) == 1, na.rm = TRUE),
    criprejneg = rowSums(dplyr::select(., b66, b67, b68, b69, b70) == 1, na.rm = TRUE),
    prej_nds = ifelse(criprejneg > 0, 1, 0),
    
    auth_events = rowSums(dplyr::select(., a74, a28, a65) == 1, na.rm = TRUE),
    criauthneg = rowSums(dplyr::select(., b74, b28, b65) == 1, na.rm = TRUE),
    auth_nds = ifelse(criauthneg > 0, 1, 0),
    
    # Other specific negative events
    oth_events = rowSums(dplyr::select(., a25, a26, a50, a51, a23) == 1, na.rm = TRUE),
    ...
    criothneg = rowSums(dplyr::select(., crireadneg, cricomneg, cridrugneg, cripdrugneg, crikidneg) == 1, na.rm = TRUE),
  ...
  mutate(
    ...
    # --- Financial groups (ordered) ---
    neg_fin_groups = case_when(
      is.na(crifinneg_num) ~ "No Neg Fin",
      crifinneg_num == 0   ~ "No Neg Fin",
      crifinneg_num %in% 1:2 ~ "1-2 Neg Fin",
      crifinneg_num >= 3   ~ "3+ Neg Fin"
    ),
    
    # --- Relationship groups (ordered) ---
    neg_rel_groups = case_when(
      is.na(crirelneg_num) ~ "No Neg Rel",
      crirelneg_num == 0   ~ "No Neg Rel",
      crirelneg_num == 1   ~ "1 Neg Rel",
      crirelneg_num >= 2   ~ "2+ Neg Rel"
    ),
  
    # treat only exact "1" (or numeric 1 if it ever is) as "Neg Home"
    neg_home_groups = if_else(home_nds_chr == "1", "Neg Home", "No Neg Home")
  ) %>%
  
  # make the new categoies factors with appropriate levels 
  mutate(
    neg_fin_groups = factor(neg_fin_groups,
                            levels = c("No Neg Fin","1-2 Neg Fin","3+ Neg Fin"),
                            ordered = TRUE),
    neg_rel_groups = factor(neg_rel_groups,
                            levels = c("No Neg Rel","1 Neg Rel","2+ Neg Rel"),
                            ordered = TRUE),
    neg_home_groups = fct_relevel(factor(neg_home_groups),
                                  "No Neg Home","Neg Home")  # reference = No Neg Home
  ) %>%
